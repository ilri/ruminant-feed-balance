...
![Feed balance model for ruminant livestock. A minus (−) indicates excluded biomass, while an asterisk (\*) denotes inclusion of feed concentration values in subsequent calculations. ME = Metabolizable Energy.](img/FeedBal_model.png){#fig-FeedBal_model fig-pos="H"}
...
The total biomass available as animal feed in Nigeria was `152` million tonnes (Mt) of DM, averaging `1.49` tonnes DM per hectare in the dry savannah, `1.02` tonnes DM per hectare in the forest region and `1.92` tonnes DM per hectare in the wet savannah.

In the dry savannah, grass contributed `12.5%` of the total metabolizable energy (ME), with crop residue, browse, and biomass from postharvest growth contributing `57.1%`, `0.18%`, and `30.3%`, respectively. In the forest zone, grass accounted for `26.4%`, while with crop residue, browse, and biomass from postharvest growth contributing `27.0%`, `31.5%`, and `15.0%`. In the wet savannah, grass provided `13.4%` of the total ME, crop residue contributed `55.1%`, browse `2.6%`, and biomass from postharvest growth `28.9% `(@fig-Total_ME).

![Metabolisable energy (ME) availability for ruminant livestock from feed sources in 2023 (TJ ME/year). Uncertainty due to ME concentration represented by error bands.](img/Total_ME.png){#fig-Total_ME fig-pos="H"}

The adequacy of locally available metabolisable energy for ruminant livestock was most constrained in the (Agro)pastoral sahel (30% of requirements locally available), Forest mixed, Northern mixed and Southern mixed zones (@fig-Requirements_me).

![Energy balance of ruminant livestock from feeds over time by zone (J ME available / J ME required/year). Uncertainty due to ME concentration and livestock parameters represented by grey bands. Dashed line at 1 where feed is sufficient for all estimated requirements (maintenance, growth, locomotion, draught power, gestation and lactation) for the zone.](img/Requirements_me.png){#fig-Requirements_me fig-pos="H"}
