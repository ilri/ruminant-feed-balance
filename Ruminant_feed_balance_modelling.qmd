...
![Feed balance model for ruminant livestock. A minus (−) indicates excluded biomass, while an asterisk (\*) denotes inclusion of feed concentration values in subsequent calculations. ME = Metabolizable Energy.](FeedBal_model.png){#fig-FeedBal_model fig-pos="H"}
...
The total biomass available as animal feed in Nigeria was 3.5 trillion tonnes (Tt) of DM, averaging 24 tonnes DM per hectare in the dry savannah, 58 tonnes DM per hectare in the forest region and 45 tonnes DM per hectare in the wet savannah.

In the dry savannah, grass contributed `92%` of the total metabolizable energy (ME), with crop residue, browse, and biomass from postharvest growth contributing `4%`, `2%`, and `2%`, respectively. In the forest zone, grass accounted for `50%` of the total ME, while browse contributed `50%`. The contributions of crop residue and biomass from postharvest growth in this zone were negligible. In the wet savannah, grass provided `84%` of the total ME, crop residue contributed `2%`, browse `12%`, and biomass from postharvest growth `1% `(@fig-Total_dm).

![Metabolisable energy (ME) availability for ruminant livestock from feed sources in 2023 (TJ ME/year). Uncertainty due to ME concentration represented by error bands.](Total_dm.png){#fig-Total_dm fig-pos="H"}

The adequacy of locally available metabolisable energy for ruminant livestock in was most constrained in the (Agro)pastoral sahel zone (30% of requirements locally available) and the northern mixed zone where requirements were met with no deficits or surplus (100% of requirements) (@fig-Requirements_me).

![Energy balance of ruminant livestock from feeds over time by zone (J ME available / J ME required/year). Uncertainty due to ME concentration and livestock parameters represented by grey bands. Dashed line at 1 where feed is sufficient for all estimated requirements (maintenance, growth, locomotion, draught power, gestation and lactation) for the zone.](Requirements_me.png){#fig-Requirements_me fig-pos="H"}
