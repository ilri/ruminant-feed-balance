...
for(year in yearList){
  ...
  if(year %in% c("2020", "2021", "2022")){
    ...
    PcChangeShoats <- raster(paste0(spatialDir, "/inputs/GLW4/pcChange2020_",year,"Shoats.tif"))
    PcChangeCattle <- raster(paste0(spatialDir, "/inputs/GLW4/pcChange2020_",year,"Cattle.tif"))
    
    lvReq <- sum(horseDonkeyIntake_model_MJ, (cattleIntake_model_MJ + (cattleIntake_model_MJ*PcChangeCattle)), (shoatsIntake_model_MJ + (shoatsIntake_model_MJ*PcChangeShoats)), na.rm = T)
    
    writeRaster(lvReq, paste0(Outputs_dir, "/lvReqTotal", year, ".tif"), overwrite=TRUE)
    
    lv_List[[year]] <- sum(horseDonkeyIntake_model_MJ, (cattleIntake_model_MJ + (cattleIntake_model_MJ*PcChangeCattle)), (shoatsIntake_model_MJ + (shoatsIntake_model_MJ*PcChangeShoats)), na.rm = T)
