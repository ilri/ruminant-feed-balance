...
for(admin_level in admin_levels){
  ...
  write_sf(aoi, paste0(outdir2, "/aoi", admin_level, ".shp"), append = FALSE)
}

aoi0 <- read_sf(paste0(outdir2, "/aoi0.shp"))
p0 <- ggplot() + 
  geom_sf(data = aoi0, colour = "black", show.legend = F) + 
  coord_sf(xlim = c(2.1, 15.1), ylim = c(3.8, 14.3), expand = FALSE)

aoi1 <- read_sf(paste0(outdir2, "/aoi1.shp"))
p1 <- ggplot() + 
...
We use FEWSNET livelihood zones, the most recent version is available on <https://shapefiles.fews.net/LHZ/NG_LHZ_2018.zip>
...
# Read in data as sf
lhz <- st_read(paste0(outdir, "/NG_LHZ_2018.shp"), quiet = TRUE)

ggplot() + 
  geom_sf(data = aoi0, colour = "black", show.legend = F) + 
  geom_sf(data = lhz, aes(fill = LZNAMEEN)) + 
  coord_sf(xlim = c(2.1, 15.1), ylim = c(3.8, 14.3), expand = FALSE) + 
...
Download the most recent version of `burned areas` data by UNEP-WCMC and IUCN (2023) from <https://land.copernicus.eu/en/products/vegetation/burnt-area-v3-1-monthly-300m>.
