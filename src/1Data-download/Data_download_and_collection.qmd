...
We create a new folder under `ruminant-feed-balance`, name it `AdminBound` and assign it the name `outdir`. We can download administrative boundaries of world countries with the `geodata` package (Hijmans et al. 2023). Here we use `geodata` to download the administrative boundaries of Nigeria from [GADM](https://gadm.org), and store the data in `AdminBound` folder. We convert it to a `simple feature` and assign it the name `aoi`.
